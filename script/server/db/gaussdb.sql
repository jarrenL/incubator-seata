...
-- -------------------------------- The script used when storeMode is 'db' (GaussDB centralized) --------------------------------
-- GaussDB is PostgreSQL-compatible, DDL is adapted from postgresql.sql.
...
INSERT INTO public.distributed_lock (lock_key, lock_value, expire) SELECT 'AsyncCommitting',  ' ', 0 WHERE NOT EXISTS (SELECT 1 FROM public.distributed_lock WHERE lock_key = 'AsyncCommitting');
INSERT INTO public.distributed_lock (lock_key, lock_value, expire) SELECT 'RetryCommitting',  ' ', 0 WHERE NOT EXISTS (SELECT 1 FROM public.distributed_lock WHERE lock_key = 'RetryCommitting');
INSERT INTO public.distributed_lock (lock_key, lock_value, expire) SELECT 'RetryRollbacking', ' ', 0 WHERE NOT EXISTS (SELECT 1 FROM public.distributed_lock WHERE lock_key = 'RetryRollbacking');
INSERT INTO public.distributed_lock (lock_key, lock_value, expire) SELECT 'TxTimeoutCheck',   ' ', 0 WHERE NOT EXISTS (SELECT 1 FROM public.distributed_lock WHERE lock_key = 'TxTimeoutCheck');
